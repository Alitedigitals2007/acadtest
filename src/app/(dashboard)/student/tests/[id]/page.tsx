...
export default function StudentTestInterface() {
        ...
        fetch(`/api/tests/${params.id}`)
          ...
          .then((tData) => {
            ...
            if (found.numQuestions && questions.length > found.numQuestions) {
              questions = questions.slice(0, found.numQuestions);
            }
  ...
  const startTest = () => {
    ...
    const saved = localStorage.getItem(`test_${params.id}_time`);
    ...
    if (saved) {
      ...
      localStorage.setItem(`test_${params.id}_time`, String(Date.now()));
      setTimeLeft(durationSeconds);
    }
    if (savedAnswers) {
      try { setAnswers(JSON.parse(savedAnswers)); } catch { /* ignore */ }
    }
    try { document.documentElement.requestFullscreen(); setFullscreen(true); } catch { /* ignore */ }
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (params.id) {
        localStorage.setItem(`test_${params.id}_answers`, JSON.stringify(answersRef.current));
      }
    }, 10000);
    return () => clearInterval(interval);
  }, [params.id, showInstructions, submitted]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.removeItem(`test_${params.id}_time`);
        setSubmitted(true);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-3 sm:px-6 py-2 sm:py-3">
        ...
        <div className="flex gap-3 mt-2 sm:hidden">
          {test.enableCalculator && (
            <button onClick={() => setShowCalculator(!showCalculator)} className="text-xs text-indigo-600 font-medium">
              {showCalculator ? "Hide Calculator" : "Show Calculator"}
            </button>
          )}
          <button onClick={toggleFullscreen} className="text-xs text-indigo-600 font-medium">
